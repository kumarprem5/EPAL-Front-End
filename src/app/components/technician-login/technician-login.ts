...
import { TechanicainLoginAuth } from '../../services/techanicain-login-auth';
...
@Component({
  ...
  imports: [CommonModule,
    FormsModule,
    ReactiveFormsModule, Header],
  templateUrl: './technician-login.html',
  styleUrl: './technician-login.css',
})
export class TechnicianLogin {
 ...
 loginData = {
  ...
  constructor(
    private authService: TechanicainLoginAuth,
        ...
        next: (res: any) => {
          ...
          if (res.status === 'SUCCESS' && res.code === '0') {
            ...
            if (token) {
              ...
              this.router.navigate(['/techanician/dashboard']);
            } else {
