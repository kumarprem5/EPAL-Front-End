...
@Component({
  selector: 'app-analyst-general-info',
  imports: [CommonModule,FormsModule,ReactiveFormsModule],
  templateUrl: './analyst-general-info.component.html',
  styleUrl: './analyst-general-info.component.css',
})
export class AnalystGeneralInfoComponent implements OnInit{
  ...
  loadGeneralInformation() {
    ...
    this.loading = true;
    this.analystService.getGeneralInformationByReportNumber(this.sample.reportNumber)
      .subscribe({
        next: (response) => {
  ...
  addNewItem() {
    ...
    this.saving = true;
    this.analystService.addGeneralInformation(newItem).subscribe({
      next: (response) => {
        if (response?.status === 'SUCCESS') {
          this.addForm.reset();
  ...
  saveEdit(item: GeneralInformationItem) {
    ...
    this.saving = true;
    this.analystService.updateGeneralInformation(updatedItem).subscribe({
      next: (response) => {
        if (response?.status === 'SUCCESS') {
          this.editMode[item.id!] = false;
          this.loadGeneralInformation();
  ...
  deleteItem(item: GeneralInformationItem) {
    ...
    this.saving = true;
    this.analystService.deleteGeneralInformation(item.id).subscribe({
      next: (response) => {
        if (response?.status === 'SUCCESS') {
          this.loadGeneralInformation();
